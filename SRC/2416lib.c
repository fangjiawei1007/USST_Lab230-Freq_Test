...
///Timer0  定义变量*************************************************////
extern int Timer0_count,ShangXian_count,Timer2_count,Timer0_SJ;
extern int flag_ShangXian_T[NUMBER];
extern int number;
...
void Timer0_Init(void)
{
	...
	rTCFG0 &= ~(0xff << 0); 
	rTCFG0 |= (110 << 0); 	// Dead zone=0, Prescaler0=10(0xf)   Timer 0 1 共用
	rTCFG1 &= ~(0xf << 0); 
	rTCFG1 |= (0x3 << 0); 	// Interrupt, Mux0=1/16
	//rTCNTB0 = 37500;    		// Timer input clock Frequency = PCLK / {prescaler value+1} / {divider value}	1 Hz
	//rTCNTB0 = 375;    		// Timer input clock Frequency = PCLK / {prescaler value+1} / {divider value}	100 Hz -- 10ms
	rTCNTB0 = 3750;
	//Timer0_SJ=rTCNTB0;
	//rTCMPB0 = 20000;
	Timer0_Start();	
...
void Timer0_Start(void)
{
	...
	rINTMSK1 &=~(BIT_TIMER0);
	tmp = rTCON & (~(0xf << 0));	// dead zone Disable
...
{
	static U32	enter_times=0;
	static U8	init_status=0;
	static U32	jidianqi_time=0;
	if (jidianqi_start == 1)
		enter_times ++;
	if ( enter_times  >= (jidianqi_fre) || init_status == 0){
		if (jidianqi_start == 1){
			rGPEDAT ^= (1<<7);
			jidianqi_time ++;
			if (jidianqi_time >=2){
	...
	else if (jidianqi_start == 0){
		init_status = 0;
	}
	if (jidianqi_qingling == 1)
		jidianqi_num=0;
  /* Timer0_count++;
  
  if(Timer0_count>=Timer0_count_SX)     // 调试中,暂定上限值为500
  {
    
    Timer0_count=0;
    ShangXian_count++;
    flag_ShangXian_T[number] = 1;  
  } */
	x1_fre_display = InPulseCount[1]*10;
	x2_fre_display = InPulseCount[2]*10;
	InPulseCount[1]=InPulseCount[2]=0;
